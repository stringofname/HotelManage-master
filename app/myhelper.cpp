...
{
    QSqlQuery query;//新建一个QSqlQuery对象query，然后他负责对数据库的所有操作！包括读，写，删除，添加等等。
    QString currentDateTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss ddd");
    qDebug() <<currentDateTime;

//用query.prepare()和query.bindValue()插入不到数据库里，不知道为什么
//    query.prepare("INSERT INTO logblog (time, do, can, user)) "
//                  "VALUES (:time :do, :can, :User)");
//      query.bindValue(":time)",currentDateTime);
//      query.bindValue(":do",Trigger); //bindValue绑定
//      query.bindValue(":can",BlogContent);
//      query.bindValue(":user",UserName);
//      query.exec();

//用VALUES ('"+currentDateTime+"', '"+。。可插入到数据库中

     qDebug()<<query.exec("INSERT INTO "+tablename+" (time, do, can, user) VALUES ('"+currentDateTime+"', '"+Trigger+"', '"+BlogContent+"', '"+UserName+"')");
     qDebug() <<"update!";

}

void myHelper::customeMyLoginBlog(QString tablename, QString UserName)
{
    QSqlQuery query;
    QString sql = "update "+tablename+" set CustomeName = '"+UserName+"' where custometype='顾客';";
    qDebug() <<sql;
    //query.exec(sql);
