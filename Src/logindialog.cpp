...
void LoginDialog::InitForm()
{
    ...
    //ui->labCompany->setText(tr("欢迎登录，顾客可自行注册使用"));
    ...
    if(ui->radioButtonAdmin->isChecked())
    {
        ...
        else
        {
            ...
            if(model.rowCount() == 1)
            {
                ...
                myHelper::MyLoginBlog("log","登录","登录系统","管理员");     //写入系统日志
    ...
    else if(ui->radioButtonCustomer->isChecked())
    {
        ...
        else
        {
            ...
            if(customePwd == UserPwd)
            {
                ...
                Myapp::WriteConfig();           //写进配置文件
                //写到数据库中的log表中
                myHelper::MyLoginBlog("log","登录系统","顾客",ui->txtUserName->text());     //写入系统日志
                //写到数据库中的customelogblog表中，表示当前登录的顾客
